classdef LEDController < handle
    ...
    methods
        ...
        %% functions to control divider
        %Set divider pattern param is a 16 binary bit pattern string.
        %Example: '1100110011001100', in which 1 means on and 0 means off. 
        function setDividerPattern(obj,param)
            if  ~(obj.serialPort == 0)
                fprintf(obj.serialPort, ['DIGITAL ', param]);
                %if obj.docheckstatus, [~,status] = checkControllerStatus(obj); end
            end
        end
        %turn on means open the divider
        function turnOnDivider(obj)
            if  ~(obj.serialPort == 0)
                % The first 0 means all panels, the second 0 means all
                % quadrants
        ...
        %turn off means close the divider, this is default states
        function turnOffDivider(obj)
            if  ~(obj.serialPort == 0)
